...
public class SyncItem {
    ...
    public long getSyncSize() {
        ...
        long fromSize = fromArr.length == FSConfig.FOLDER_INFO_LEN ? 0 : Long.parseLong(fromArr[FSConfig.SIZE_INDEX]);
        if (isCopy() || 0 == fromSize) {
            return fromSize;
        }
        long size = 0;
        String[] fromContent = fromArr[FSConfig.CONTENT_INDEX].split(FSConfig.CONTENT_SEP);
        String[] toContent = to.split(FSConfig.INFO_SEP)[FSConfig.CONTENT_INDEX].split(FSConfig.CONTENT_SEP);
