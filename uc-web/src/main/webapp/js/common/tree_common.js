...
function searchFilter(node, key) {
    ...
    if (!validateNull(node.extensionNumber)) {
        if (node.extensionNumber.toLocaleLowerCase().indexOf(key) != -1) {
            return true;
        }
    }
    if (!validateNull(node.phoneIP)) {
        if (node.phoneIP.toLocaleLowerCase().indexOf(key) != -1) {
            return true;
        }
    }
    return false;
