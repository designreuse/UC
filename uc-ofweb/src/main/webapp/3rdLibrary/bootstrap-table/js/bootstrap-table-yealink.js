...
!function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        // 新加
        formatYealinkTotal: function(totalRows) {
            return sprintf('A total of %s records, ', totalRows);
        },
        formatYealinkPerPage: function(pageSize) {
            return sprintf('Per page show %s record.  ', pageSize);
        },
        formatYealinkPageNum: function(pageNumber) {
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $.each(this.options.columns, function (i, column) {
            ...
            if (column.checkbox) {
                if (!that.options.singleSelect && that.options.checkboxHeader) {
                    text = '<input name="btSelectAll" class="regular-checkbox" type="checkbox" /><label for="btSelectAll"></label>';
                }
        ...
        this.$header.find('label').off('click').on('click', function () {
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        html.push(
            ...
            this.options.formatYealinkTotal(this.options.totalRows),
            '</span>');
        ...
        html.push(this.options.formatYealinkPerPage(pageNumber.join('')));
        html.push('</span>');

        // 第几页
        html.push('<span class="page-number-list">');
        var yeaPageNumber = [
                '<span class="btn-group dropdown">',
                '<button type="button" class="btn btn-sm dropdown-toggle" data-toggle="dropdown">',
        ...
        $.each(pageNumberList, function (i, page) {
            var active = page === that.options.pageNumber ? ' class="active"' : '';
            yeaPageNumber.push(sprintf('<li %s><a href="javascript:void(0)">%s</a></li>', active, page));
        });
        yeaPageNumber.push('</ul></span>');

        html.push('&nbsp;&nbsp;&nbsp;'+this.options.formatYealinkPageNum(yeaPageNumber.join('')));
        ...
        $pageNumberList = this.$pagination.find('.page-number-list a');
        ...
        $pageNumberList.off('click').on('click', $.proxy(this.onPageNumberListChange, this));
    ...
    BootstrapTable.prototype.onPageNumberListChange = function (event) {
        var $this = $(event.currentTarget);

        $this.parent().addClass('active').siblings().removeClass('active');
        this.options.pageNumber = +$this.text();
        this.$toolbar.find('.page-number').text(this.options.pageNumber);
        this.updatePagination();
    };
    ...
    BootstrapTable.prototype.initBody = function () {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (that.options.columns[j].checkbox || that.options.columns[j].radio) {
                    ...
                    text = ['<td class="bs-checkbox">',
                        ...
                        sprintf(' checked="%s"', value ? 'checked' : undefined) + ' /><label for="' +that.options.selectItemName+i+ '"></label>',
                        //sprintf('<label for="%s"></label>', that.options.selectItemName+j),
                        '</td>'].join('');
                } else {
        ...
        this.$body.find('label').off('click').on('click', function () {
...
//            $(this).parents('tr')[checked ? 'addClass' : 'removeClass']('selected');
    ...
    // added by peng
    BootstrapTable.prototype.changeUrlAndRresh = function (newUrl) {
